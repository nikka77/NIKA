...
19. **Forms/nindō/stats des têtes d'affiche** — Naruto 30 stars (databooks Fandom), OP ~15 (Mugiwara/Yonko + rang de prime en geste de repli), DB étendre DB_FORMS — lourd, par lots de 5. *(06/08 soir — **QUASI-CLOS : les trois volets sont livrés, 3 restes ponctuels.** Trois vagues appliquées : vague C (**34 posées** : 26 Naruto + 10 OP, 2 sautées déjà pourvues, 62 replis motivés dans `data/audits/stars-curation.json`, trace avant dans `stars-curation-trace.json`) ; corrections sanji/chopper appliquées en base (`stars-corrections-trace.json`) ; **vague D 11/11 posées** (`data/audits/stars-vague-d.json` + trace avec instantanés avant) — les 4 Naruto rejetées re-validées et posées (might-guy, boruto-uzumaki, haku, mitsuki) + 7 Dragon Ball (gokuu-black, trunks, kame-sennin, krillin, bardock, vegetto, yamcha) → **DB = 18 fiches à frise**, 11 replis motivés (bulma, beerus, 17, 18, ten-shin-han, arale, whis : aucun état nommé par l'œuvre — c'est canon, pas un manque). Les deux décisions de Dan + **l'arbitrage 06/08 « OMETTRE les stats post-databook, jamais reconduire »** sont actés dans `stars-contrat.json` (regle_de_visibilite_DURE révisée, clé `arbitrage_post_databook`) et appliqués en base (0 stat sur tout DB, stats omises sur la 8ᵉ Porte de Guy et l'Edo Tensei de Haku). Vérifiés sur pages : might-guy (3 tuiles stylisées, radar 31,5/40 databook, 0 radar sur la 8ᵉ Porte), krillin (frise sans radar) ; build 310/310, sonde verte. Assumés sans suite : nami/portgas-d-ace/katakuri sans frise possible, shanks/boa-hancock/jinbe/barbe-blanche nindō+prime seuls. **Restes (3)** : (a) **son-goten** — seule fiche rejetée non re-posée : retirer les champs age/height/weight des formes (stats interdites) et la phrase télépathie inventée de la caption Goten (Mini), tout le reste est vérifié — corrections détaillées dans les replis de `stars-vague-d.json` ; (b) ✅ **soldé 06/08 par décision de Dan** : luffy/zoro **gardent** leurs stats estimées, désormais marquées « stats estimées — non canon » (mention SVG + aria-label, prop `estime` pilotée par `universe !== 'Naruto'`) ; le bug FormePanel est corrigé à la source (`CharacterZone.tsx` calcule `statLabels`/`estime` une fois et les passe aux DEUX panneaux — plus d'axes Naruto sur les fiches OP). Vérifié : might-guy canon sans mention (31,5/40), luffy remappé PUI/TEC/AGI/… avec mention SSR + DOM. **Ne jamais re-proposer la purge.** Zéro nouvelle stat estimée reste la règle ; (c) **committer** : `stars-vague-d.json`, `stars-vague-d-trace.json` et le `stars-contrat.json` révisé sont non commités dans le dépôt canonique.)*
