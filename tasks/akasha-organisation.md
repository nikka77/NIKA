...
19. **Forms/nindō/stats des têtes d'affiche** — Naruto 30 stars (databooks Fandom), OP ~15 (Mugiwara/Yonko + rang de prime en geste de repli), DB étendre DB_FORMS — lourd, par lots de 5. *(06/08 — **le geste signature n'est plus inerte sur les stars.** Lot des 45 stars du contrat `data/audits/stars-contrat.json` traité : **36 fiches curées et validées** (26 Naruto + 10 OP), **34 posées** en base (fusion ajout-seulement, attributes AVANT tracés dans `data/audits/stars-curation-trace.json`, vérif post-écriture 34/34), 2 sautées déjà pourvues (nico-robin, usopp), **62 replis motivés** documentés dans `data/audits/stars-curation.json` — dont Minato jamais chiffré par un databook (les 35,5/40 en ligne sont apocryphes), Tobi/Jinchūriki de Jūbi jamais chiffrés, formes post-ch. 402 sans stats. **Deux décisions de Dan actées et livrées côté front** : (1) une forme SANS image est légitime — tuile stylisée (initiale Bebas sur dégradé d'accent) dans ArcFrieze + CharacterZone, `normalizeForms` assoupli, FormePanel titré « Forme N » ; (2) **zéro stat estimée** — seules les lignes databook canon Naruto portent un radar, les stars OP reçoivent frise + prime sans radar (vérifié sur trafalgar-law). Vérif : stats de 4 fiches recollationnées axe par axe contre le wiki (80 valeurs, 0 écart), naruto-uzumaki non-régressé (10 sprites intacts), build 310/310 vert, sonde schéma verte, committé. **Reste** : (a) 6 fiches rejetées à l'amont pour erreurs factuelles ponctuelles, corrections documentées dans les replis, à corriger puis réappliquer — might-guy, boruto-uzumaki, haku, mitsuki, sanji, tony-tony-chopper (NB : sanji et chopper ont en base une version d'un lot antérieur qui porte l'erreur signalée) ; (b) 3 stars OP sans forme canon possible, laissées sans frise (nami, portgas-d-ace, katakuri-charlotte) et 4 posées nindō/prime seuls (shanks, boa-hancock, jinbe, barbe-blanche) ; (c) volet Dragon Ball (étendre DB_FORMS) non entamé ; (d) rafraîchir la « regle_de_visibilite_DURE » de `stars-contrat.json` (décrit encore l'ancien rejet des formes sans visuel) et corriger le gotcha statLabels OP avant toute curation OP avec stats ; (e) trancher avec Dan « omettre vs reconduire la ligne DB3 » pour les formes post-databook (Byakugō de Sakura, 8ᵉ Porte de Guy, Deidara Edo Tensei, formes de guerre de Shikamaru/Gaara).)*
