...
export const LIMITES_FOURNISSEURS = {
  ...
  // jetons 20 000 → 200 000 (audit 02/08) : la doc du palier gratuit donne 500 000 jetons/min
  // PAR MODÈLE (help.mistral.ai). Une relecture en réserve ~7 000 : à 20 000, notre fenêtre ne
  // laissait passer que 2-3 appels/min alors que requetes:12 en autorise 12 — encore un guichet
  // fermé par NOUS. Aligné sur le repli mistral:, très en dessous du documenté.
  'mistral/mistral-large-latest':    { requetes: 12, jetons: 200_000, fenetre: 60 },
  // Second couloir Mistral gratuit (audit 02/08) : les compteurs du palier gratuit sont PAR
  // MODÈLE — mistral-small ne partage pas le guichet de mistral-large. Un juge de plus à coût
  // nul ; si le compteur s'avérait partagé, le 429 fermerait le couloir une heure, c'est géré.
  'mistral/mistral-small-latest':    { requetes: 12, jetons: 200_000, fenetre: 60 },
  ...
  // COULOIR D'ASSURANCE (02/08) — PAYANT, dernier de toutes les rotations. Les ~9,45 $ de crédit
  // OpenRouter ne servent à rien en inférence courante (0,00039 $/verdict contre 0,000156 $ chez
  // DeepInfra flex, comparé sur 10 candidats) : leur usage efficace est d'être une SECONDE maison
  // de facturation, activée seulement si DeepInfra tombe ou s'épuise. ~24 000 verdicts de réserve.
  // Les modèles payants ne comptent PAS dans le quota :free de 1 000/jour — pas de parJour.
  'openrouter/mistralai/mistral-small-24b-instruct-2501': { requetes: 30, jetons: 300_000, fenetre: 60 },
  ...
  // ── Anthropic — l'ARBITRE SUPRÊME (02/08). Facturé au jeton comme DeepInfra, mais avec un
  // guichet quotidien VOLONTAIRE : 400 litiges/jour ≈ 2,60 $ maximum — le plafond de dépense
  // est un choix de bourse, pas une limite du fournisseur. Haiku 4.5 suffit largement au
  // barème d'arbitrage (les litiges du 02/08 tranchés en atelier l'ont montré).
  'anthropic/claude-haiku-4-5': { requetes: 30, jetons: 300_000, fenetre: 60, parJour: 400 },
