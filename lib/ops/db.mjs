...
export function clientSite() {
  if (!_site) {
    _site = createClient(
      ...
      {
        auth: { persistSession: false },
        // Timeout ferme : sous restrictions egress, Supabase peut PENDRE indéfiniment
        // (Cloudflare 522 après 60-100 s) — une requête sans limite gelait le tick lent
        // de la console /ops, donc tout son rafraîchissement (03/08). Mieux vaut échouer
        // en 20 s et laisser l'appelant décider que gelér le monde.
        global: { fetch: (u, o) => fetch(u, { ...o, signal: AbortSignal.timeout(20_000) }) },
      },
    );
  }
  return _site;
